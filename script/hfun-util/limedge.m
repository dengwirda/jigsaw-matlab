function [ffun,flag] = limedge(edge,elen,ffun,dfdx,imax)
%LIMEDGE impose "gradient-limits" on a function defined over 
%an undirected graph.
%   [FNEW] = LIMEDGE(EDGE,ELEN,FFUN,DFDX,ITER) computes a
...
%
%   [FNEW,FLAG] = LIMEDGE(...) also returns a boolean FLAG,
...
%---------------------------------------------- basic checks    
    if ( ~isnumeric(edge) || ...
         ...
         ~isnumeric(dfdx) || ...
         ~isnumeric(imax) )
        error('limedge:incorrectInputClass' , ...
...
%---------------------------------------------- basic checks
    if (ndims(edge) ~= +2 || ...
        ...
        numel(dfdx) ~= +1 || ...
        numel(imax) ~= +1 )
        error('limedge:incorrectDimensions' , ...
...
%---------------------------------------------- basic checks
    if (dfdx < +0. || imax < +0)
        error('limedge:invalidInputArgument', ...
